...
通过设定 begin 和 end 来确定有效帧的范围。
如果 p_interval 中只有一个值，那么采用中心裁剪（center crop），去除一定比例的边缘帧。
如果 p_interval 中有两个值，随机选择一个 p 值，并根据该值确定裁剪的范围。
裁剪后的数据存储在 data 中。
调整大小（Resize）:

将裁剪后的数据转换为 PyTorch 张量。
调整数据的维度，使其适应 PyTorch 的 F.interpolate 函数，这可能涉及到对数据的排列和视图变换。
使用双线性插值（bilinear interpolation）对数据进行调整大小。这一步的目的可能是为了将数据调整为指定的窗口大小。
最后将调整大小后的数据重新转换为 NumPy 数组。
'''
...
#函数的目的是通过在数据中引入随机旋转，增加模型的鲁棒性，使其能够处理不同角度的输入。
def random_rot(data_numpy, theta=0.3):
    ...
    #uniform_均匀分布产生随机数
    rot = torch.zeros(3, dtype=torch.float64).uniform_(-theta, theta)
    #复制为300 3
