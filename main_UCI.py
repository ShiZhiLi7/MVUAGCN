    ...
    def train(self, epoch):
        ...
        for batch_idx, (x_data, label) in enumerate(process):
            ...
            if self.arg.flag == 3:   
                ...
                feats2 = x_data[:,:, self.arg.feats_split[0]:]
                feats2 = torch.tensor(feats2,dtype=torch.float32).to(DEVICE)
    
               
                timer['dataloader'] += self.split_time()
                out1,out2 = self.model(self.edges, feats1, feats2)
            
                if self.arg.Dili == 1:
                    if self.arg.DS == 1:
                        alpha = self.DS_Combin([out1+1,out2+1])
                    else:
                        alpha = out1 + out2 + 1
                else:
                    alpha = out1 + out2
        ...
        with torch.no_grad():
            ...
            for batch_idx, (x_data, label) in enumerate(process):
                ...
                if self.arg.flag == 3:   
                    ...
                    if self.arg.Dili == 1:
                        if self.arg.DS == 1:
                            output = self.DS_Combin([out1+1,out2+1])
                        else:
                            output = out1 + out2 + 1
                    else:
                        output = out1 + out2
